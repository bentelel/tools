...
class FileHandler:
    ...
    def SetCheckChar(self, character: str) -> None:
        self.check_char = character.encode(self.encoding)
    ...
    def SetDataFrameFromFilePath(self) -> None:
    ...
    async def AnalyzeDataframe(self) -> None:
            ...
            for col in self.dataframe:
                filtered_df = await loop.run_in_executor(
                    ...
                    lambda col=col: self.dataframe[
                        self.dataframe[col].astype(str).str.contains(self.check_char, na=False)]
                )
                if not filtered_df.empty:
...
async def load_file_and_set_dataframe() -> None:
    ...
    fileHandler.SetFilePath(file_path[0])
    fileHandler.SetDataFrameFromFilePath()
...
async def analyzer_click() -> None:
    try:
        ...
        analyze_button.set_visibility(False)
        await fileHandler.AnalyzeDataframe()
...
def show_data_rows(col_name: str) -> None:
    ...
    filtered_df = fileHandler.dataframe[fileHandler.dataframe[col_name].astype(str).str.contains(fileHandler.check_char, na=False)].head(10)
...
if __name__ in ("__main__", "__mp_main__"):
    ...
    with ui.tab_panels(tabs, value=main_page).classes('w-full') as panels:
        with ui.tab_panel(main_page):
            app.on_shutdown(
                kill_script)  # sys exit is triggered here, for some reason that does not cleanly exit the script.. the script is exited by crashing though
            ...
            ui.input(label='Character to check for', value=',').bind_value(fileHandler, 'check_char')
